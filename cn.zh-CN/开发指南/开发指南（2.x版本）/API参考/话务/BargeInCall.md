...
调用BargeInCall实现强插，将用户加入到jobId指定的会话中。

## 调试

[您可以在OpenAPI Explorer中直接运行该接口，免去您计算签名的困扰。运行成功后，OpenAPI Explorer可以自动生成SDK代码示例。](https://api.aliyun.com/#product=CCC&api=BargeInCall&type=RPC&version=2020-07-01)

## 请求参数

|名称|类型|是否必选|示例值|描述|
|--|--|----|---|--|
|Action|String|是|BargeInCall|系统规定参数。取值：BargeInCall。 |
|BargedUserId|String|是|user-test2@ccc-test|被强插的座席ID。 |
...
|TimeoutSeconds|Integer|否|60|超时时间，呼叫在经过该参数指定的时间仍然未接通的情况下，则主动挂断。单位秒。 |
...
|SkillGroupId|String|skg-test1@ccc-test|该通话相关技能组。呼入场景下，技能组由IVR中路由的排队队列指定，在呼出场景下，技能组是座席签入的第一个技能组。 |
...
```
http(s)://[Endpoint]/?Action=BargeInCall
&BargedUserId=user-test2@ccc-test

...
[您可以在OpenAPI Explorer中直接运行该接口，免去您计算签名的困扰。运行成功后，OpenAPI Explorer可以自动生成SDK代码示例。](https://api.aliyun.com/#product=CCC&api=MonitorCall&type=RPC&version=2020-07-01)

## 请求参数

|名称|类型|是否必选|示例值|描述|
|--|--|----|---|--|
|Action|String|是|MonitorCall|系统规定参数。取值：MonitorCall。 |
|DeviceId|String|是|ACC-YUNBS-1.0.10-b18bp53c1736c2914176a801ei90|座席端提供的唯一ID，用来表示一个座席工作台。 |
|InstanceId|String|是|ccc-test|呼叫中心实例ID。 |
|MonitoredUserId|String|是|user-test@ccc-test|被监听的座席的ID。 |
|UserId|String|是|monitor-test@ccc-test|呼叫中心实例的用户ID，实例内唯一。 |
|TimeoutSeconds|Integer|否|30|超时时间，呼叫在经过该参数指定的时间仍然未接通的情况下，则主动挂断。 |
...
```
http(s)://[Endpoint]/?Action=MonitorCall
&DeviceId=ACC-YUNBS-1.0.10-b18bp53c1736c2914176a801ei90
&InstanceId=ccc-test
&MonitoredUserId=user-test@ccc-test
&UserId=monitor-test@ccc-test
&<公共请求参数>
```
